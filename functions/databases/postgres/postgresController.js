      ...
      this.pgConnect()
        ...
        .then(({ database, done }) => {
          ...
          if (phone.substring(0, 2) !== '+1') {
            ...
            reject(new Error('phone number must be formatted correctly'))
          }
          ...
          if (!phone.match(/^\+?[1-9]\d{1,14}$/g)) {
            done();
            reject(new Error('phone number invalid'));
          }
            ...
            .then((service) => {
              ...
              database.query('INSERT INTO twoauthusers(userID, sid, phone) VALUES($1, $2, $3) RETURNING *', [userID, sid, phone])
                .then((user) => {
                  ...
                  // needs to grab the user info from the results object
                  resolve(user.rows[0]);
                })
                .catch((err) => {
